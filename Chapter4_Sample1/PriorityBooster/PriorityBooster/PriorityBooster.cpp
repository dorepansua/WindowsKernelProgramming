...
_Use_decl_annotations_ NTSTATUS PriorityBoosterDeviceControl(PDEVICE_OBJECT,
                                                             PIRP Irp) {
  ...
  switch (stack->Parameters.DeviceIoControl.IoControlCode) {
    case IOCTL_PRIORITY_BOOSTER_SET_PRIORITY: {
      ...
      auto data =
          (ThreadData*)stack->Parameters.DeviceIoControl.Type3InputBuffer;
      if (data == nullptr) {
      ...
      __try {
        ...
        PETHREAD Thread;
        status =
            PsLookupThreadByThreadId(ULongToHandle(data->ThreadId), &Thread);
